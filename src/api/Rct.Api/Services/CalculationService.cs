...
///
/// <para><b>Zugriff</b>: jeder AUTHENTIFIZIERTE Nutzer darf das Kalkulationsbuch lesen. Die
/// EINZIGE Prüfung ist, dass das Buch existiert UND als Kalkulationsbuch markiert ist
/// (<see cref="Book.IsCalculation"/>); sonst <see cref="KeyNotFoundException"/> → 404. Kein
/// CourseAccess-/Gruppen-/IsPublic-Gating (RCT kennt weder Kurse noch Gruppen), und keine
/// anonyme/öffentliche Lese-Öffnung — der Modus ist ausschließlich eingeloggt.</para>
/// </summary>
public class CalculationService
{
    ...
    /// letzten Speichern; ein hängender/schlafender Client soll die Summe nicht sprengen.</summary>
    ...
    /// <summary>
    /// Zugriffsregel des Kalkulations-Modus: ein Buch ist zugänglich, wenn es EXISTIERT und ein
    /// Kalkulationsbuch ist (<see cref="Book.IsCalculation"/>). Alles andere ist „nicht gefunden"
    /// (<see cref="KeyNotFoundException"/> → 404) — auch ein vorhandenes Nicht-Kalkulationsbuch,
    /// damit über diesen Pfad keine Lösung (<c>Moves</c>) herausgeht.
    /// </summary>
    private async Task EnsureBookAccessAsync(int bookId, CancellationToken ct = default)
    {
        var isCalculation = await _db.Books
            .Where(b => b.Id == bookId)
            .Select(b => (bool?)b.IsCalculation)
    ...
    /// (Punkte IMMER mit ihrem Maximum). Reihenfolge wie im Kurs (Round → Id).</summary>
    ...
    /// <item>Die Festlegung wird GESETZT: ein anderer Zug verschiebt sie, derselbe ist ein No-op;
    /// das Zurücknehmen macht der Client per <c>ClearChoice</c>.</item>
    /// </list>
    /// </summary>
    ...
    private static bool ApplySeconds(CalculationTree tree, ICalcMetaInput dto)
    {
        ...
        // unterschlagen, weil der nächste Retry `already == requested` sähe. Im Normalfall
        // (requested ≤ cap) ist already + delta == requested — unverändert.
    ...
    public async Task DeleteTreeAsync(int userId, int bookPuzzleId, bool isAdmin, CancellationToken ct = default)
    {
        var tree = await _db.CalculationTrees
            .FirstOrDefaultAsync(t => t.UserId == userId && t.BookPuzzleId == bookPuzzleId, ct);
        if (tree == null) return;
        await EnsureBookAccessAsync(tree.BookId, ct);
